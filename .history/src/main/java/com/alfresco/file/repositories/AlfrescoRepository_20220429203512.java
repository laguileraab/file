...
public class AlfrescoRepository {
  ...
  public String uploadFile(MultipartFile multipartFile) {
    ...
    Mono<Object> responseObject = webClient.post()
        ...
        .with("name", "Test" + new Random().nextInt())
        .with("nodeType", "cm:content")
        .with("relativePath", "Cliente")
   //     .with(map))
        )
        .exchangeToMono(response -> {
          if (response.statusCode().equals(HttpStatus.CREATED) | response.statusCode().equals(HttpStatus.OK){
