...
public class AlfrescoRepository {
  ...
  public FileDownload downloadFile(String id) throws InterruptedException, IOException{
    ...
    WebClient webClient = WebClient.builder()
        ...
        .retrieve()
        //.bodyToFlux(BodyExtractors.toDataBuffers())
        ...
        ClientResponse cResponse = webClient.get().uri(downloadURL)
        .accept(MediaType.APPLICATION_OCTET_STREAM)
        .exchange()
        .block();

    final int statusCode = cResponse.rawStatusCode();
    // check HTTP status code, can throw exception if needed
    // ....

    Flux<DataBuffer> body = cResponse.body(BodyExtractors.toDataBuffers());
    ...
    DataBufferUtils.write(body, osPipe)
       ...
       // Flux<DataBuffer> body = response.body(BodyExtractors.toDataBuffers())
        ...
        file.setFile(isPipe.readAllBytes());
