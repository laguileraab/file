...
public class AlfrescoRepository {
    ...
    public Mono<File> searchFile(String id) {
        ...
        String children = "/children";
                ...
                RequestHeadersSpec body = uriSpec.uri(URI.create(path + api + tenant + visibility + repo + versions + node + children));

        // RequestBodySpec bodySpec = uriSpec
        //         .uri(URI.create(path + api + tenant + visibility + repo + versions + node + children));
        
        RequestHeadersSpec<?> headersSpec = body.bodyValue("entries");
        ...
        Mono<Object> response = headersSpec.exchangeToMono(responseL -> {
